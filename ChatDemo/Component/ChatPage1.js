...
class ChatPage extends React.Component {
  ...
  getSendedMessage = () => {
    ...
    return new Promise(resolve => {
      axios
        .post(
          'https://us-central1-cloudfunctiondemo-d2104.cloudfunctions.net/chatApp/api/DemoGetChatData',
          data,
        )
        .then(res => {
  ...
  getChat2 = () => {
    ChatMessages = [];
    this.state.sendMessages.map(item =>
      ChatMessages.push({
    ...
    this.state.receiveMessage.map(item =>
      ChatMessages.push({
        message: item.message,
        dateTime: item.datetime._seconds,
        status: 'recieve',
  ...
  sendMsg = () => {
    ...
    axios
      .post(
        'https://us-central1-cloudfunctiondemo-d2104.cloudfunctions.net/chatApp/api/insChat',
        data,
      )
      .then(res => {
        // console.log(res);
  ...
  getFirebaseData = () => {
    ...
    if (
      ...
      this.state.receiveMessage.length === 0 &&
      this.state.wholeChat.length === 0
    ) {
      ...
      qry.onSnapshot(doc => {
        doc.forEach(d => {
          this.setState({
            wholeChat: [
              ...this.state.wholeChat,
              {message: d.data().message, status: 'sent'},
            ],
          });
        });
      });
      ...
      qry1.onSnapshot(doc => {
        doc.forEach(d => {
          this.setState({
            wholeChat: [
              ...this.state.wholeChat,
              {message: d.data().message, status: 'recieve'},
            ],
          });
        });
      });
      ...
      <View style={{flex: 1}}>
        <View style={{flex: 1, flexDirection: 'row'}}>
          <View
            ...
            <Image
              source={{
                uri:
                  'https://firebasestorage.googleapis.com/v0/b/collageappdemo.appspot.com/o/User_profile%2Fimg1580980542056?alt=media&token=03d02880-e6e6-4a1e-93bf-ee904dd5e26a',
              }}
          ...
          <View style={{flex: 4}}>
            <TouchableOpacity
              onPress={() => alert(this.props.phoneno)}
              style={{flex: 1, height: null, width: null}}>
              <Text style={{fontSize: 17, marginTop: 10}}>
                {this.props.reciverName}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={{flex: 9.2}}>
          <ScrollView style={{flex: 1, padding: 10}}>
            {this.state.wholeChat.map(chatMessages => (
              <Text
                style={{
                  fontSize: 20,
                  padding: 10,
                  alignSelf:
                    chatMessages.status === 'sent' ? 'flex-end' : 'flex-start',
                }}>
        ...
        <View style={{flex: 0.8, flexDirection: 'row'}}>
          <View style={{flex: 9, justifyContent: 'center'}}>
            <TextInput
              ...
              onChangeText={text => this.setState({userMsg: text})}
