    ...
    getNewContent=(newsHeadlineArray)=>{
        ...
        return new Promise((resolve=>{
            ...
            newsHeadlineArray.map((data,index)=>{
                ...
                if(index<17) {
                    axios.post('https://covidapi123.herokuapp.com/covid/api/getIndiaNews', urldata, {
                        headers: {
                ...
                <SafeAreaView style={{flex:1}}>
                    ...
                    <ScrollView style={{flex:1}}>
                    {
                        this.state.newsContentArray.map((data,index)=>{
                            return(
                                <View style={{height:h*.15,marginTop:h*0.010,padding:h*0.010,backgroundColor:index%2==1 ?'#ECEDEE':'white',}}>
                                    <TouchableOpacity onPress={()=>this.showDetailNews(index)}>
                                    <Text numberOfLines={2} ellipsizeMode='tail' style={{fontSize:normalize(15)}}>{data.title}</Text>
                                    </TouchableOpacity>
                                </View>
                            )
                        ...
                        <Text style={{textAlign:'right',fontSize:normalize(12)}}>Source:-TOI(Times of India)</Text>
                                    ...
                                    <View style={{flex:1,padding:h*.010}}>
                                        ...
                                        <View>
                                            <Image
                                                ...
                                                source={{uri: this.state.newsContentArray[this.state.tempIndex].image}}
                                                />
                                            <Text style={{fontSize:normalize(18),marginTop:h*.010}}>{this.state.newsContentArray[this.state.tempIndex].description}</Text>
                                            <Text style={{fontSize:normalize(15),marginTop:h*.020,textAlign:'right'}}>Source-{this.state.newsContentArray[this.state.tempIndex].source}</Text>
